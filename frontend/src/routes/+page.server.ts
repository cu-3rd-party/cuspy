...
export const load: PageServerLoad = async ({ parent }) => {
	...
	if (sessionFlow?.user) {
		redirect(307, sessionFlow.status === 'approved' ? '/target-intel' : sessionFlow.status === 'pending' ? '/waiting-clearance' : sessionFlow.status === 'rejected' ? '/agent-id?mode=edit' : '/agent-id');
	}
