...
test('approved user can submit kill report and admin can moderate it', async ({ browser }) => {
	...
	await clientContext.addCookies([
		{ name: 'backend-access-token', value: 'approved-token', url: 'http://127.0.0.1:4173' }
	]);
	await adminContext.addCookies([
		{ name: 'backend-access-token', value: 'admin-token', url: 'http://127.0.0.1:4173' }
	]);
	...
	await clientPage
		.getByLabel('MODUS_OPERANDI')
		.fill('Shadowed target through Sector 7 and confirmed clean elimination.');
	await clientPage.getByRole('button', { name: 'FINALIZE_REPORT' }).click();
	await expect(
		clientPage.getByText('Kill report submitted for RAZOR_WIND_ENFORCER.')
	).toBeVisible();
	...
	await expect(adminPage.getByRole('button', { name: /RAZOR_WIND_ENFORCER/ })).toContainText(
		'confirmed'
	);
